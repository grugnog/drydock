        ...
        stage('Code linting') {
            steps {
                script {
                    ...
                    sh 'docker run -it -v "$(pwd)":/workdir -w /workdir -e PHP_CS_FIXER_IGNORE_ENV=1 unibeautify/php-cs-fixer fix -v --dry-run --stop-on-violation --using-cache=no tools/getconfig.php'
                }
            }
        }
